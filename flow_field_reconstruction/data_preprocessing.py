...
def process_csv_row(csv_row: pd.DataFrame, output_data: pd.DataFrame, all_blade_sensors: Dict[str, Dict[str, Any]], other_sensors: Dict[str, Any]) -> None:
    ...
    for key in output_data.keys():
        if key.endswith("1") or "B1" in key:
            ...
            if "Time" in all_blade_sensors["blade1"]:
                all_blade_sensors["blade1"]["Time"].append(csv_row[key].values[0])
                all_blade_sensors["blade2"]["Time"].append(csv_row[key].values[0])
                all_blade_sensors["blade3"]["Time"].append(csv_row[key].values[0])
                other_sensors[key].append(csv_row[key].values[0])
            else:
                all_blade_sensors["blade1"]["Time"] = [csv_row[key].values[0]]
                all_blade_sensors["blade2"][key] = [csv_row[key].values[0]]
                all_blade_sensors["blade3"][key] = [csv_row[key].values[0]]
...
def process_run_directory(run_directory: Path, interim_path: Path, processed_path: Path, time_step: int) -> None:
    ...
    for i in tqdm(range(len(ts_file.t)), desc=f"Processing {run_directory.name}"):
        if i*time_step >= len(output_data):
            max_idx = i
            break
        csv_rows = output_data.iloc[i*time_step:i*time_step+time_step, :]
